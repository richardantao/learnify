...
export const editYear = _id => (dispatch, getState) => {
    ...
    axios.get(`http://localhost:3000/v1/years/${_id}`/*, tokenConfig(getState)*/)
    .then(res => dispatch({
        type: EDIT_YEAR,
        payload: res.data
    }))
...
export const updateYear = (_id, year) => (dispatch, getState) => {
    ...
    axios.put(`http://localhost:3000/v1/years/${_id}`, year/*, tokenConfig(getState)*/)
    .then(res => dispatch({
        type: UPDATE_YEAR,
        payload: res.data
    }))
...
export const deleteYear = _id => (dispatch, getState) => {
    axios.delete(`http://localhost:3000/v1/years/${_id}`/*, tokenConfig(getState)*/)
    .then(res => dispatch({
        type: DELETE_YEAR,
        payload: _id
    }))
